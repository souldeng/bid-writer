...
const PROMPTS:Record<string,string>={
  bg:"深入分析建设背景：①国家及地方数字化政策形势，包括国家领导层讲话、会议、文件等；②当前领域现状与主要不足（列举3-4点）；③建设必要性和紧迫性。层次清晰，有说服力。",
    ...
    for(let i=0;i<fileArray.length;i++){
      ...
      try{
        ...
        if(d.success){
          ...
          fail++;
          onToast(`「${f.name}」：${d.error||"上传失败"}`,"err");
        }
      }catch(e){
        fail++;
        onToast(`「${f.name}」网络错误，请检查网络后重试`,"err");
      }
    }
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:"#fff",borderRadius:8,padding:20,border:`0.5px solid ${T.ink15}`}}>
            ...
            <div
              ...
              style={{
                border:`1.5px dashed ${dragOver?T.gold:uploading?T.blue:T.ink15}`,
                borderRadius:6,padding:"28px 16px",textAlign:"center",
                cursor:uploading?"not-allowed":"pointer",
                background:dragOver?T.goldBg:uploading?T.blueBg:T.ink06,
              ...
              <div style={{fontSize:13,color:T.ink,fontWeight:500,marginBottom:4}}>
                {uploading
                  ?(uploadProgress||"正在处理文件…")
                  :"点击选择 或 拖拽文件到此处"}
              </div>
              <div style={{fontSize:11,color:T.ink40}}>PDF · DOCX · TXT · MD · 最大 10MB</div>
              {/* 【修复2a】input 完全独立，不在可点击 div 内层嵌套触发混乱 */}
              <input
                ref={fileRef}
                type="file"
                multiple
            ...
            {/* 【新增】上传中进度提示条 */}
            {uploading&&(
              <div style={{marginTop:10,padding:"8px 12px",background:T.blueBg,borderRadius:4,border:`0.5px solid ${T.blue}33`,fontSize:11,color:T.blue}}>
                ⏳ {uploadProgress||"处理中，请稍候…"}
              </div>
            )}
          </div>
          ...
          <div style={{padding:"10px 14px",background:T.ink06,borderRadius:6,border:`0.5px solid ${T.ink15}`,fontSize:11,color:T.ink40,lineHeight:1.7}}>
            ⚠️ 当前版本：文件上传后知识库保存在服务端内存中。页面刷新或服务重启后需重新上传文件。
          </div>
        </div>

        <div style={{background:"#fff",borderRadius:8,border:`0.5px solid ${T.ink15}`,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`0.5px solid ${T.ink15}`,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:13,fontWeight:600,flex:1}}>已上传文件</span>
            <Badge color="gray">{docs.length} 个文件</Badge>
            {/* 【新增】手动刷新按钮 */}
            <button
              onClick={fetchDocs}
              style={{padding:"3px 10px",borderRadius:3,border:`0.5px solid ${T.ink15}`,background:"transparent",color:T.ink40,fontSize:11,fontFamily:"inherit",cursor:"pointer"}}
              title="刷新列表">
              ↻ 刷新
            </button>
          </div>
          <div style={{flex:1,overflowY:"auto"}}>
            {loading
              ?<div style={{padding:24,textAlign:"center",color:T.ink40,fontSize:13}}>加载中…</div>
              :docs.length===0
                ?<div style={{padding:40,textAlign:"center"}}>
                  <div style={{fontSize:32,marginBottom:10}}>📭</div>
                  <div style={{fontSize:13,color:T.ink40,marginBottom:8}}>知识库为空，上传文件后 AI 生成质量将显著提升</div>
                  <div style={{fontSize:11,color:T.ink15}}>支持 PDF、DOCX、TXT、MD 格式</div>
                </div>
                :docs.map(doc=>{
                  ...
                  return(
                    <div key={doc.id} style={{padding:"14px 18px",borderBottom:`0.5px solid ${T.ink06}`,display:"flex",gap:12,alignItems:"flex-start"}}>
                      ...
                      <button onClick={()=>deleteDoc(doc.id,doc.name)} style={{background:"none",border:"none",cursor:"pointer",color:T.ink40,fontSize:16,padding:4,borderRadius:3,flexShrink:0}} title="删除">🗑</button>
                    </div>
                  );
                })
            }
